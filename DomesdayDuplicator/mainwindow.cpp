    ...
    log(log)
{
    ...
    // Load the application's configuration settings file (needed for theme detection)
...
void MainWindow::StartCapture()
{
    ...
    if (configuration->getCaptureFormat() == Configuration::CaptureFormat::tenBitPacked)
    {
        captureFilePath += ".lds";
             ...
             configuration->getCaptureFormat() == Configuration::CaptureFormat::sixteenBitSigned_Half ||
             configuration->getCaptureFormat() == Configuration::CaptureFormat::sixteenBitSigned_Quarter)
    {
        captureFilePath += ".s16";  // All 16-bit raw formats use .s16 extension
    }
    ...
    {
        captureFilePath += ".flac";
    }
    else
    {
        captureFilePath += ".s16";  // Default to signed 16-bit extension
    }
    ...
    {
        qDebug() << "MainWindow::StartCapture(): Starting transfer - 16-bit (will compress to LDF post-capture)";
        captureFormat = UsbDeviceBase::CaptureFormat::Signed16Bit;
    }
    else if (configuration->getCaptureFormat() == Configuration::CaptureFormat::flacDirect)
    {
...
void MainWindow::StartAudioCapture(const std::filesystem::path& rfFilePath)
{
    ...
    if (pid == 0) {
        ...
        FILE* logf = fopen("/tmp/ddd_fmedia.log", "w");
...
{
    int themeStyle = configuration ? configuration->getThemeStyle() : 0; // Default to Auto if config not loaded yet
...
{
    // Check various system indicators for dark theme
    
    // Method 1: Check application palette brightness
    QPalette defaultPalette = qApp->palette();
    QColor windowColor = defaultPalette.color(QPalette::Window);
    double brightness = (windowColor.red() * 0.299 + windowColor.green() * 0.587 + windowColor.blue() * 0.114) / 255.0;
    
    if (brightness < 0.5) {
        return true; // Dark theme detected via palette
    }
    
    // Method 2: Check environment variables (Linux)
    QString desktopSession = qgetenv("XDG_CURRENT_DESKTOP").toLower();
    QString gtkTheme = qgetenv("GTK_THEME").toLower();
    QString qtStyleOverride = qgetenv("QT_STYLE_OVERRIDE").toLower();
    
    if (gtkTheme.contains("dark") || qtStyleOverride.contains("dark")) {
        return true;
    }
    
    // Method 3: Check for common dark theme names in style
    QString currentStyle = qApp->style()->objectName().toLower();
    if (currentStyle.contains("dark") || currentStyle.contains("breeze-dark") || currentStyle.contains("adwaita-dark")) {
        return true;
    }
    
    // Method 4: Check Qt application attributes for high contrast/dark themes
    if (qApp->testAttribute(Qt::AA_UseHighDpiPixmaps)) {
        // Additional heuristic: if using high DPI and system palette is dark
        QColor textColor = defaultPalette.color(QPalette::WindowText);
        double textBrightness = (textColor.red() * 0.299 + textColor.green() * 0.587 + textColor.blue() * 0.114) / 255.0;
        if (textBrightness > 0.5) {
            return true; // Light text suggests dark background
        }
    }
